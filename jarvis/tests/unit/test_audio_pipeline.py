...
def test_piper_speaker_interrupt_drains_multiple_pending_replies(tmp_path: Path) -> None:
    """Queued stale speech must not resume after an interruption."""
    first_play_started = threading.Event()
    release_playback = threading.Event()
